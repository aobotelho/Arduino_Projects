...
void setup(){
  ...
  client.subscribe((char*)(DISCOVER_CALLBACK + String(NODE_ID)).c_str());
...
boolean connectMQTT(){
  unsigned int counter=0;
  while(!client.connect((char*)String(NODE_ID).c_str())){
    delay(100);
    if(counter == 20){
      #if USE_SERIAL
...
boolean registerDevice(){
    ...
    while(!nodeRegistered){
      ...
      client.publish((char*)DISCOVER_TOPIC,(char*)registerPayload.c_str());
      //maintain MQTT connection
      client.loop();
      delay(1500);    
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(String(topic)==(DISCOVER_CALLBACK+String(NODE_ID)) && !nodeRegistered){
    #if USE_SERIAL
      Serial.println("Discover topic received");
    #endif
    if(payloadStr == "Registered"){
      nodeRegistered = true;
      client.unsubscribe((char*)(DISCOVER_CALLBACK + String(NODE_ID)).c_str());
      #if USE_SERIAL
    ...
    #endif 
    client.publish((char*)(STATUS_CALLBACK + String(NODE_ID)).c_str(),(outputStatus?"ON":"OFF"));
  }
